...
export default function Home(){
 ...
 const save=async(e:React.FormEvent)=>{e.preventDefault();setMessage('');if(!form.supplier.trim()){setMessage('Inserisci il fornitore.');return}if(!photo){setMessage('Scatta la foto dell’etichetta o del documento di consegna.');return}if(!supabase){setMessage('Database non configurato.');return}
  ...
  const {data,error}=await supabase.from('trace_arrivals').insert({supplier:form.supplier.trim(),photo_path:path,product_name:null,producer_lot:null}).select('internal_lot,delivery_date').single()
  if(error){setMessage(error.message);return}
  setForm(empty);setPhoto(null);setMessage(`Registrato: lotto ${data.internal_lot} · consegna ${new Date(data.delivery_date+'T12:00:00').toLocaleDateString('it-IT')}`);await load()
 }
 if(view==='new')return <main><Top/><button className="back" onClick={()=>setView('home')}><ChevronLeft/> Dashboard</button><section className="panel"><p className="eyebrow">ARRIVO MERCE</p><h2>Registra lotto</h2><p className="muted">Fotografa l'etichetta o il documento della merce e indica il fornitore. TRACE assegna automaticamente lotto di arrivo e data di consegna.</p><form onSubmit={save} className="form simple"><label className="cameraBox"><span>Foto merce / etichetta *</span><input className="cameraInput" type="file" accept="image/*" capture="environment" onChange={e=>setPhoto(e.target.files?.[0]||null)}/><div className="cameraButton"><Camera size={26}/><b>{photo?'FOTO ACQUISITA':'APRI FOTOCAMERA'}</b><small>{photo?photo.name:'Su smartphone e iPad si apre la fotocamera posteriore'}</small></div></label><Field label="Fornitore *" value={form.supplier} onChange={v=>setForm({...form,supplier:v})} placeholder="Nome fornitore"/><div className="autoInfo"><div><span>LOTTO DI ARRIVO</span><b>Automatico</b></div><div><span>DATA CONSEGNA</span><b>{new Date().toLocaleDateString('it-IT')}</b></div></div><button className="primary">REGISTRA ARRIVO</button>{message&&<div className="message">{message}</div>}</form></section></main>
